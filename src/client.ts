...
export default class ZStatusClient {
  ...
  /* Integrations */
  getIntegrations(params: APIIntegrationSearchArgs) {
    const searchParams = new URLSearchParams(params as any);
    const url = `${this.baseUrl}/integrations?${searchParams.toString()}`;
    return axios
      .get(url, {
        headers: this.getHeaders(),
      })
  ...
  createIntegration(integration: CreateIntegrationProps) {
  ...
  updateIntegration(integrationId: string, updates: UpdateIntegrationProps) {
    const url = `${this.baseUrl}/integrations/${integrationId}`;
    return axios
      .put(url, {
        headers: this.getHeaders(),
